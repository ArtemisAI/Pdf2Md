 ...
 * Provides real-time progress updates during RTX 3060 GPU-optimized transcription
 */
...
export class ProgressReporter {
   ...
   * Report completion
   */
  reportComplete(taskId: string, message = 'Enhanced transcription completed successfully'): void {
  ...
  private getStageInfo(stage: ProgressStage, details?: string): { progress: number; message: string } {
    switch (stage) {
      case 'initializing':
        return { progress: 5, message: details || 'Initializing enhanced audio transcription...' };
      
      case 'detecting_gpu':
        return { progress: 10, message: details || 'Detecting RTX 3060 GPU configuration...' };
      
      case 'loading_model':
        return { progress: 20, message: details || 'Loading GPU-optimized transcription model...' };
      
      case 'processing_audio':
        return { progress: 30, message: details || 'Processing audio file...' };
      
      case 'validating_format':
        return { progress: 35, message: details || 'Validating audio format...' };
      
      case 'starting_transcription':
        return { progress: 45, message: details || 'Starting GPU-accelerated transcription...' };
      
      case 'transcribing':
        return { progress: 70, message: details || 'Transcribing audio with RTX 3060 optimization...' };
      ...
      case 'switching_to_cpu':
        return { progress: 25, message: details || 'GPU error detected, switching to CPU fallback...' };
      
      case 'optimizing_memory':
        return { progress: 30, message: details || 'Optimizing GPU memory usage...' };
