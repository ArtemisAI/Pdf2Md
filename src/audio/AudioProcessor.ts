 ...
 * Handles format validation, conversion, and audio file operations
 */
...
`;
    ...
    const tempScriptPath = path.join(os.tmpdir(), `convert_${Date.now()}.py`);
    await fs.promises.writeFile(tempScriptPath, scriptContent);
    return tempScriptPath;
  }
...
`;
    ...
    const tempScriptPath = path.join(os.tmpdir(), `metadata_${Date.now()}.py`);
    await fs.promises.writeFile(tempScriptPath, scriptContent);
    return tempScriptPath;
  }
...
`;
    ...
    const tempScriptPath = path.join(os.tmpdir(), `chunk_${Date.now()}.py`);
    await fs.promises.writeFile(tempScriptPath, scriptContent);
    return tempScriptPath;
  }
