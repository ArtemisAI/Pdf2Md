 ...
 * Optimized for RTX 3060 (12GB VRAM) and GPU detection based on Audio_Transcribe_3 patterns
 */
...
export class ConfigManager {
  ...
  static getOptimalConfig(): TranscriptionConfig {
    ...
    return {
      ...
      // Performance settings based on device (RTX 3060 optimized)
   ...
   * Includes RTX 3060 specific optimizations (12GB VRAM)
   */
   ...
   * RTX 3060 specific optimizations - medium model is optimal for 12GB
   */
  private static selectOptimalModel(deviceInfo: DeviceInfo): TranscriptionConfig['modelSize'] {
    ...
    if (gpuMemory >= 6) return 'base';          // 6GB GPUs
  ...
  private static checkCUDAAvailability(): boolean {
    ...
    // Default assumption - let Python detect CUDA
    return cudaVisible || torchCuda || nvidiaSmi || true;
  }
...
if torch.cuda.is_available():
  ...
  private static isRTX3060(): boolean {
    ...
    return gpuName.includes('rtx 3060') || gpuName.includes('rtx3060') || 
           gpuName.includes('geforce rtx 3060');
  }
   ...
   */
  static getConfigSummary(config: TranscriptionConfig): string {
    return `Model: ${config.modelSize}, Device: ${config.device}, ` +
           `Batch: ${config.batch_size}, Type: ${config.torch_dtype}`;
  }
}
